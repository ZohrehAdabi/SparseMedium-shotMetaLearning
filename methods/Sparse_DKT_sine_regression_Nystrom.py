...
from data.sine_loader import Task_Distribution , get_batch, get_unnormalized_label
# from configs import kernel_type
from collections import namedtuple
import torch.optim
#Check if tensorboardx is installed
try:
    #tensorboard --logdir=./Sparse_DKT_Nystrom_Sine_Loss/ --host localhost --port 8091
...
class Sparse_DKT_sine_regression_Nystrom(nn.Module):
    def __init__(self, backbone, kernel_type='rbf', add_rvm_mll=False, add_rvm_mse=False, lambda_rvm=0.1, normalize=False, lr_decay=False, f_rvm=True, scale=True, config="0000", align_threshold=1e-3, gamma=False, n_inducing_points=12, random=False, 
        ...
        super(Sparse_DKT_sine_regression_Nystrom, self).__init__()
        ## GP parameters
    ...
    def get_model_likelihood_mll(self, train_x=None, train_y=None):
        if(train_x is None): train_x=torch.ones(self.num_induce_points, 40).cuda() #2916: size of feature z
    ...
    def init_summary(self, id):
        if(IS_TBX_INSTALLED):
            ...
            if not os.path.isdir('./Sparse_DKT_Nystrom_Sine_Loss'):
                os.makedirs('./Sparse_DKT_Nystrom_Sine_Loss')
            writer_path = './Sparse_DKT_Nystrom_Sine_Loss/' + id #+'_'+ time_string
    ...
    def train_loop_fast_rvm(self, train_loader, epoch, n_support, n_samples, optimizer):
        self.model.train()
        self.likelihood.train()
        self.feature_extractor.train()
        inputs, labels = train_loader.sample_task().sample_data(n_samples, noise=0.1)
        ...
        if self.kernel_type=='rbf':
            if (epoch+1)%2==0 :
                print(Fore.LIGHTRED_EX,'[%02d] - Loss: %.4f ML %.4f RVM ML: %.4f RVM MSE: %.4f  MSE: %.3f noise: %.4f outputscale: %.3f lengthscale: %.3f' % (
                    epoch, loss.item(), -mll.item(), -rvm_mll.item(), rvm_mse.item(), mse.item(),
    ...
    def test_loop_fast_rvm(self, val_loader, n_support, n_samples, test_person, optimizer=None): # no optimizer needed for GP

        self.model.eval()
        self.likelihood.eval()
        self.feature_extractor.eval()

        inputs, targets = val_loader.sample_task().sample_data(n_samples, noise=0.1, sort=True)
        ...
        query_indices = np.sort(indices[n_support:])
    ...
    def train(self, stop_epoch, n_support, n_samples, optimizer):
        ...
        train_range = (1, 100)
        amplitude = (0.1, 5)
        train_loader     = Task_Distribution(amplitude_min=amplitude[0], amplitude_max=amplitude[1], 
                                  phase_min=0.0, phase_max=np.pi, 
                                  x_min=train_range[0], x_max=train_range[1], 
                                  family="sine")
        val_loader = Task_Distribution(amplitude_min=amplitude[0], amplitude_max=amplitude[0], 
                                   phase_min=0.0, phase_max=np.pi, 
                                   x_min=train_range[0], x_max=train_range[1], 
                                   family="sine")
        ...
        for epoch in range(stop_epoch):
            ...
            if  self.f_rvm:
                mll = self.train_loop_fast_rvm(train_loader, epoch, n_support, n_samples, optimizer)
                ...
                if epoch%1==0:
                    ...
                    for t in range(val_count):
                        mse, mse_, sv_count, mse_r = self.test_loop_fast_rvm(val_loader, n_support, n_samples, t,  optimizer)
                        mse_list.append(mse)
    ...
    def test(self, n_support, n_samples, optimizer=None, test_count=None): # no optimizer needed for GP
        ...
        test_loader = Task_Distribution(amplitude_min=amplitude[0], amplitude_max=amplitude[0], 
                                   phase_min=0.0, phase_max=np.pi, 
                                   x_min=train_range[0], x_max=train_range[1], 
                                   family="sine")
        for t in range(test_count):
            ...
            print(f'test #{t}')
            if self.f_rvm:
                mse, mse_, num_sv, mse_r = self.test_loop_fast_rvm(test_loader, n_support, n_samples, t,  optimizer)
                num_sv_list.append(num_sv)
                
            elif self.random:
                mse = self.test_loop_random(n_support, n_samples, t,  optimizer)
            elif not self.f_rvm:
                mse = self.test_loop_kmeans(n_support, n_samples, t,  optimizer)
            else:
                ValueError()
