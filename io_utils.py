...
def parse_args(script):
    ...
    parser.add_argument('--tol_rvm'             , default=1e-4, type=float, help='Max iteration of RVM in inner loop') 
    ...
    if script == 'train':
        ...
        parser.add_argument('--best'            , action='store_true', help='use best model or last model')
        parser.add_argument('--DKT_features'    , action='store_true', help='use DKT features in Baseline')
        parser.add_argument('--Baseline_features', action='store_true', help='use Baseline features in DKT')
...
def get_resume_file(checkpoint_dir):
    ...
    filelist =  [ x  for x in filelist if os.path.basename(x) not in ['best_model.tar', 'best_model_rvm.tar'] ]
