...
class Gaussian_Task():
    ...
    A sine wave data distribution object with interfaces designed for MAML.
    """
    def __init__(self, outscale, lengthscale, xmin, xmax, size=100, sort=False):
        self.outscale = outscale
        self.lengthscale = lengthscale #sigma2
        self.xmin = xmin
        self.xmax = xmax
        self.x = np.random.uniform(self.xmin, self.xmax, size=size)
        if(sort): self.x = np.sort(self.x)
        self.K = outscale * np.exp (-1/(2*self.lengthscale)*np.abs(self.x[:,np.newaxis]-self.x[:,np.newaxis].T)**2)
        self.mean = np.zeros(size) # Zero mean vector

    def true_function(self, x):
        """
        Compute the true function on the given x.
        """
        return  np.random.multivariate_normal(mean=self.mean, cov=self.K, size=1)
    ...
    def sample_data(self, size=1, noise=0.0, sort=False):
        ...
        y = self.true_function()
        if(noise>0): y += np.random.normal(loc=0.0, scale=noise, size=y.shape)
        x = torch.tensor(self.x, dtype=torch.float).unsqueeze(1)
